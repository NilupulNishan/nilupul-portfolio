...
Nav is deliberately split: the first three items (`Home`, `About`, `Experience`) are Home anchors, and everything after them (`Brands`, `Afterlife`, `Lab`, `Contact`) is a route.

**`Brands` points at `/projects`.** The label and the URL differ on purpose: that route renders brand cards, the live profile view and the promotions form - no engineering work. The code projects are the `CaseStudies` section on Home. While it was labelled "Projects" a visitor hunting for source code clicked it, found content channels, and had no reason to keep looking. Only the label changed, so `vercel.json`'s `/channels` -> `/projects` redirect and `public/sitemap.xml` still hold.

**Nav items may declare `children`** (`portfolio.js`), which `renderNavItem` turns into a sub-menu. Only `Experience` uses it today, surfacing the five Home sections that otherwise collapse under one nav item (`experience`, `case-studies`, `github`, `certificates`, `testimonials` - note `certificates`, which is the real DOM id). On desktop it is a **disclosure**, not `role="menu"`: a caret `<button>` with `aria-expanded` sits beside the parent link so the menu works by keyboard and tap, not hover alone, with Escape to close and a ~180ms close delay so a diagonal cursor movement into the panel does not dismiss it. The parent anchor still navigates to `#experience` on its own. On mobile the children render as an always-visible indented list, since there is no hover and a second tap would defeat the purpose. `Navbar` tracks `activeSection` alongside `activeNav` so the panel can show which of the five you are currently in.

Note `src/index.css` still carries an unused `.nav-dropdown` / `.nav-dropdown-trigger` / `.creator-dropdown` block from an earlier menu; the new one is named `.nav-submenu` to avoid `.nav-dropdown { position: relative }` silently breaking its positioning. Keeping all anchors ahead of all routes stops the scroll highlight from jumping over a dead gap in the middle of the bar. Because those route pages are absent from the Home scroll entirely, `ExploreMore` (a card row near the end of Home) exists purely to make them discoverable without the navbar; `ContactCta` is the slim band that closes Home and links to `/contact`.
